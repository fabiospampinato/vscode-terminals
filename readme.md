...
- **Global-level terminals**: you can also define additional global-level terminals in your VSCode settings files, that will be available in every project, using the same exact configuration format, but basically prefixing every top-level key with `terminals.`, so `terminals.autorun`, `terminals.terminals` etc., the rest stays unchanged.
- **Multiplexer-level persistence**: a basic form of multiplexer-based persistence is supported, via either [GNU Screen](https://en.wikipedia.org/wiki/GNU_Screen) or [tmux](https://en.wikipedia.org/wiki/Tmux), which will persist terminals even if you close vscode, and it will reattach to them when you reopen vscode. For it to work you need to respectively have the `screen` or `tmux` commands already installed in your system.
- **Native-level persistence**: VSCode supports persisting terminals natively. To avoid conflicts this feature is automatically disabled for terminals created by this extension, and you might want to disable it globally as well, by setting the `terminal.integrated.persistentSessionReviveProcess` setting to `never`.
...
- **Env substitutions**: additionally values from the provided `env` object can be substituted by writing for example `[env:key]` or `${env:key}` somewhere. Env substitution is supported for the following configuration properties only: `description`, `cwd`, `command`, `commands`.
- **Tilde substitution**: paths starting with a tilde, like `~/Desktop` are also supported, the tilde will be automatically resolved. This is supported for the following configuration properties only: `cwd`, `shellPath`.
